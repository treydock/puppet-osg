...
describe 'osg::lcmaps_voms class:' do
  ...
  context 'with parameters defined' do
    ...
    describe file('/etc/grid-security/voms-mapfile'), node: node do
      its(:content) { is_expected.to match %r{"/GLOW/*" glow} }
      its(:content) { is_expected.to match %r{"/GLOW/chtc/*" glow1} }
      its(:content) { is_expected.to match %r{"/GLOW/Role=htpc/*" glow1} }
    end
    describe file('/etc/grid-security/grid-mapfile'), node: node do
      its(:content) { is_expected.to match %r{"/fooDN" foo} }
      its(:content) { is_expected.to match %r{"/foo" foobar} }
      its(:content) { is_expected.to match %r{"/bar" foobar} }
    end
    describe file('/etc/grid-security/ban-voms-mapfile'), node: node do
      its(:content) { is_expected.to match %r{"/cms/Role=production/*"} }
    end
    describe file('/etc/grid-security/ban-mapfile'), node: node do
      its(:content) { is_expected.to match %r{/foo/baz} }
    end
  end
